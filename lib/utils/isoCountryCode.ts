...
/**
 * Convert ISO 3166-1 alpha-2 country code to alpha-3 country code.
 */
export const isoCountryCode2To3 = (iso2Code: string): string | null => {
  if (!iso2Code || iso2Code.length !== 2) {
    return null
  }
  return ISO2_TO_ISO3[iso2Code.toUpperCase()] || null
}

export const isoCountryCode3To2 = (iso3Code: string): string | null => {
  if (!iso3Code || iso3Code.length !== 3) {
    return null
  }
  const entry = Object.entries(ISO2_TO_ISO3).find(([_, v]) => v === iso3Code.toUpperCase())
  return entry ? entry[0] : null
}

export const isISO2Code = (code: string) => {
  return /^[A-Za-z]{2}$/.test(code)
}

export const isISO3Code = (code: string) => {
